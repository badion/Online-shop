$(document).ready(function() {
	...
	$('#registrationId input[type="text"]').tooltipster({
		trigger : 'custom',
		onlyOne : false,
		position : 'right'
	});

	$('#registrationId input[type="password"]').tooltipster({
		trigger : 'custom',
		onlyOne : false,
		position : 'right'
	});

	$('#registrationId').validate({ // initialize the plugin
		...
		messages : {
			password : {
				minlength : "Minimal length of password: 5 characters",
				maxlength : "Maximal length of password: 32 characters",
			...
			firstName : {
				user : "First name is incorrect",
				required : "This field is required"
			},
			lastName : {
				user : "Second name is incorrect",
				required : "This field is required"
			},
			login : {
				minlength : "Minimal length of login: 5 characters",
				loginn : "Login is incorrect",
				required : "This field is required"
			},
			email : {
			...
			phone : {
				phonen : "Phone is incor rect",
				minlength : "Minimal length of phone: 5 characters",
				required : "This field is required"
			},
...
(function(d, f, g, b) {
	...
	function a() {
		...
		v = [ "Moz", "Webkit", "Khtml", "O", "ms" ], o = o.charAt(0)
				.toUpperCase()
				+ o.substr(1);
	...
	h.prototype = {
		init : function() {
			...
			if (q == true) {
				if ((this.options.iconDesktop == true) && (!j())
						|| ((this.options.iconTouch == true) && (j()))) {
					var m = r.attr("title");
					r.removeAttr("title");
					var p = n.options.iconTheme;
					var o = d('<span class="' + p.replace(".", "")
							+ '" title="' + m + '">' + this.options.icon
							+ "</span>");
					o.insertAfter(r);
					r.data("tooltipsterIcon", o);
					r = o
				}
				var l = d.trim(n.options.content).length > 0 ? n.options.content
						: r.attr("title");
				...
				if ((this.options.touchDevices == true) && (j())) {
					...
					if (this.options.trigger == "click") {
						r.on("click.tooltipster", function() {
							if ((r.data("tooltipster") == "")
									|| (r.data("tooltipster") == b)) {
								n.showTooltip()
							} else {
								n.hideTooltip()
							}
		...
		showTooltip : function(m) {
			...
			if ((d(".tooltipster-base").not(".tooltipster-dying").length > 0)
					&& (l.options.onlyOne == true)) {
				d(".tooltipster-base").not(".tooltipster-dying").not(
						n.data("tooltipster")).each(function() {
							...
							function() {
								l.options
										.functionBefore(
												n,
												function() {
													if ((n.data("tooltipster") !== b)
															&& (n
																	.data("tooltipster") !== "")) {
														var w = n
																.data("tooltipster");
														if (!w
																.hasClass("tooltipster-kill")) {
															var s = "tooltipster-"
																	+ l.options.animation;
															w
															...
															if ((l.options.touchDevices == true)
																	&& (j())) {
																d("body")
																		.bind(
																				"touchstart",
														...
														var s = "tooltipster-"
																+ l.options.animation;
														var r = "-webkit-transition-duration: "
																+ l.options.speed
																...
																+ l.options.speed
																+ "ms;";
														var o = l.options.fixedWidth > 0 ? "width:"
																+ l.options.fixedWidth
																+ "px;"
																: "";
														var z = l.options.maxWidth > 0 ? "max-width:"
																+ l.options.maxWidth
																+ "px;"
																: "";
														var t = l.options.interactive == true ? "pointer-events: auto;"
																: "";
														var w = d('<div class="tooltipster-base '
																+ y
																+ " "
																+ s
																+ '" style="'
																		...
																		if ((A !== B)
																			...
																			w
																					.css(
																							{
																								width : "",
																								"-webkit-transition-duration" : l.options.speed
																										+ "ms",
																								"-moz-transition-duration" : l.options.speed
																										+ "ms",
																								"-o-transition-duration" : l.options.speed
																										+ "ms",
																								"-ms-transition-duration" : l.options.speed
																										+ "ms",
																								"transition-duration" : l.options.speed
																										+ "ms",
		...
		hideTooltip : function(m) {
			...
			if ((o !== b) && (o !== "")) {
				...
				if (k == true) {
					o.clearQueue().removeClass(n + "-show").addClass(
							"tooltipster-dying").delay(l.options.speed).queue(
							function() {
								o.remove();
								p.data("tooltipster", "");
					...
					o.clearQueue().addClass("tooltipster-dying").fadeOut(
							l.options.speed, function() {
								o.remove();
								p.data("tooltipster", "");
								d("body").css("verflow-x", "");
								l.options.functionAfter(p)
		...
		positionTooltip : function(O) {
			...
			if ((A.data("tooltipster") !== b) && (A.data("tooltipster") !== "")) {
				...
				if (A.is("area")) {
					...
					var n = P.offset().left;
					var L = P.offset().top;
					var W = A.attr("coords") !== b ? A.attr("coords")
							.split(",") : b;
					if (T == "circle") {
						var N = parseInt(W[0]);
						var r = parseInt(W[1]);
				...
				function t(an, am) {
					if (((Z - d(f).scrollTop() - M - X - 12) < 0)
							&& (am.indexOf("top") > -1)) {
						ab.options.position = an;
						y = am
					}
					if (((Z + ag + M + 12 + X) > (d(f).scrollTop() + d(f)
							.height()))
							&& (am.indexOf("bottom") > -1)) {
						ab.options.position = an;
						y = am;
				...
				if (ab.options.arrow == true) {
					...
					if ((ab.options.position == "top")
							|| (ab.options.position == "top-left")
							|| (ab.options.position == "top-right")) {
						var U = parseFloat(ah.css("border-bottom-width"));
						var z = ah.css("border-bottom-color")
					} else {
						if ((ab.options.position == "bottom")
								|| (ab.options.position == "bottom-left")
								|| (ab.options.position == "bottom-right")) {
					...
					if (U !== 0) {
						...
						E = '<span class="tooltipster-arrow-border" style="'
								+ C + " " + S + ';"></span>'
					}
					ah.find(".tooltipster-arrow").remove();
					p = '<div class="' + I + ' tooltipster-arrow" style="' + ak
							+ '">' + E + '<span style="border-color:' + R
							+ ';"></span></div>';
					ah.append(p)
				}
				ah.css({
					...
					.each(function() {
						...
						case "destroy":
							...
							d(this)
									...
									.attr("title", o.data("tooltipsterContent"))
									.data("tooltipsterContent", "")
									.data("plugin_tooltipster", "")
									.off(
						...
						case "update":
							...
							break;
						case "reposition":
							d(this).data("plugin_tooltipster")
									.positionTooltip();
							break
						}
					});
